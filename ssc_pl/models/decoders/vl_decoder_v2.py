...
from ..layers import (ASPP, DeformableSqueezeAttention, DeformableTransformerLayer, LearnableSqueezePositionalEncoding,
                      TransformerLayer, Upsample)
...
class SymphoniesLayer(nn.Module):
    ...
    def __init__(self, embed_dims, num_heads=8, num_levels=3, num_points=4, query_update=True):
        super().__init__()
        self.query_image_cross_defrom_attn = DeformableTransformerLayer(embed_dims, num_heads, num_levels, num_points)
        self.scene_query_cross_attn = TransformerLayer(embed_dims, num_heads, mlp_ratio=0)
        self.scene_self_deform_attn = DeformableTransformerLayer(embed_dims,
                                                                 num_heads,
                                                                 num_levels=1,
        ...
        if query_update:
            self.query_scene_cross_deform_attn = DeformableTransformerLayer(embed_dims,
                                                                            num_heads,
                                                                            num_levels=1,
                                                                            num_points=num_points * 2,
                                                                            attn_layer=DeformableSqueezeAttention,
    ...
    def forward(self,
        ...
        scene_pos_fov = flatten_fov_from_voxels(scene_pos, fov_mask) if scene_pos is not None else None
        ...
        inst_queries = self.query_image_cross_defrom_attn(inst_queries,
                                                          feats_flatten,
                                                          query_pos=inst_pos,
                                                          ref_pts=ref_2d,
        ...
        scene_embed_fov = self.scene_query_cross_attn(scene_embed_fov, inst_queries, inst_queries, scene_pos_fov, inst_pos)
        ...
        inst_queries = self.query_scene_cross_deform_attn(inst_queries,
                                                          scene_embed_flatten,
                                                          query_pos=inst_pos,
...
class VoxelLanguageLogitLayer(nn.Module):

    def __init__(self, embed_dims, downsample_z, project_scale, num_heads=8):
        super().__init__()
        self.vl_transformer = TransformerLayer(embed_dims, num_heads, mlp_ratio=0)
        self.lv_transformer = TransformerLayer(embed_dims, num_heads, mlp_ratio=0)
    ...
    def forward(self, scene_embed, text_feats, fov_mask):
        text_feats = text_feats.unsqueeze(0)
        scene_embed_fov = flatten_fov_from_voxels(scene_embed, fov_mask)
        scene_embed_flatten, scene_shape = flatten_multi_scale_feats([scene_embed])
        scene_embed_flatten = self.vl_transformer(scene_embed_flatten, text_feats, text_feats)
        text_feats = self.lv_transformer(text_feats, scene_embed_flatten, scene_embed_flatten)
        scene_embed = index_fov_back_to_voxels(scene_embed, scene_embed_fov, fov_mask)
        # vox_feats = self.upsample(scene_embed)
        text_feats = text_feats.squeeze(0)

        inner_prod_logit = (scene_embed.permute(0, 2, 3, 4, 1) @ text_feats.t()).permute(0, 4, 1, 2, 3)
        inner_prod_logit = F.interpolate(inner_prod_logit, size=(256, 256, 32), mode='trilinear',
                                         align_corners=False).view(-1, 20, 256, 256, 32)
        return inner_prod_logit


class VisionLanguageDecoderV2(nn.Module):
    ...
    def __init__(self,
        ...
        self.layers = nn.ModuleList([
            SymphoniesLayer(embed_dims, num_levels=num_levels, query_update=True if i != num_layers - 1 else False)
            for i in range(num_layers)
        ])

        self.scene_embed = nn.Embedding(self.num_queries, embed_dims)
        self.scene_pos = LearnableSqueezePositionalEncoding((128, 128, 2), embed_dims, squeeze_dims=(2, 2, 1))
        ...
        self.logit_layer = VoxelLanguageLogitLayer(embed_dims, downsample_z, project_scale)
    ...
    def forward(self,
        ...
        if self.downsample_z != 1:
            projected_pix = interpolate_flatten(projected_pix, self.ori_scene_shape, self.scene_shape, mode='trilinear')
            fov_mask = interpolate_flatten(fov_mask, self.ori_scene_shape, self.scene_shape, mode='trilinear')
        vol_pts = pix2vox(self.image_grid,
                          depth.unsqueeze(1),
                          K,
                          E,
                          voxel_origin,
        ...
        ref_3d = self.generate_vol_ref_pts_from_masks(pred_insts['pred_boxes'], pred_masks,
                                                      vol_pts).unsqueeze(2) if pred_masks else self.generate_vol_ref_pts_from_pts(
                                                          pred_insts['pred_pts'], vol_pts).unsqueeze(2)
        ref_pix = (torch.flip(projected_pix, dims=[-1]) + 0.5) / torch.tensor(self.image_shape).to(projected_pix)
        ...
        for i, layer in enumerate(self.layers):
            scene_embed, inst_queries = layer(scene_embed, inst_queries, feats, scene_pos, inst_pos, ref_2d, ref_3d, ref_vox,
                                              fov_mask)
            if i == 2:
            ...
            if (self.training and i == 0) or i == len(self.layers) - 1:
                vox_feats = self.upsample(scene_embed)
                inner_prod_logit = self.logit_layer(scene_embed, text_feats, fov_mask)
    ...
    def generate_vol_ref_pts_from_masks(self, pred_boxes, pred_masks, vol_pts):
        ...
        pred_boxes = torch.cat([(cx - 0.5 * w), (cy - 0.5 * h), (cx + 0.5 * w), (cy + 0.5 * h)], dim=-1).int()
        pred_boxes[0::2] = pred_boxes[0::2].clamp(0, self.image_shape[1] - 1)
        pred_boxes[1::2] = pred_boxes[1::2].clamp(1, self.image_shape[1] - 1)

        pred_masks = F.interpolate(pred_masks.float(), self.image_shape, mode='bilinear').to(pred_masks.dtype)
        ...
        vol_pts = vol_pts.sum(dim=2) / pred_masks.sum(dim=2) / torch.tensor(self.scene_shape).to(vol_pts)
